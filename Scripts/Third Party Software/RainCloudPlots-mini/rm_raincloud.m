    ...
    for j = 1:n_series
        ...
        switch density_type
            ...
            case 'ks'
                ...
                [ks{i, j}, x{i, j}] = ksdensity(data{i, j}, 'npoints', n_bins(i, j), 'bandwidth', bandwidth);
...
for i = 1:n_plots_per_series
    for j = 1:n_series
        ...
        % scatter rainclouds
        h.s{i, j} = scatter(data{i, j}, -jit{i, j} + ks_offsets(i), 'MarkerFaceColor', colours(j, :), 'MarkerEdgeColor', 'none', 'SizeData', raindrop_size);
        %'MarkerFaceAlpha', 0.5
        alpha(0.5)
...
for i = 1:n_plots_per_series
    for j = 1:n_series
        ...
        h.m(i, j) = scatter(cell_means(i, j), ks_offsets(i), 'MarkerFaceColor', colours(j, :), 'MarkerEdgeColor', [0 0 0], 'SizeData', raindrop_size * 2, 'LineWidth', 2);
        % 'MarkerFaceAlpha', 1
        alpha(0.1)
    end
end
