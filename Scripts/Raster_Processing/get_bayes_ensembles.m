...
if Analyze
    ...
    for CAGindex=1:MaxCAG
        ...
        fprintf('For %i CA Neurons->Ratio of Neurons: %3.3f \n',CAGindex,PercAN(CAGindex));
    end
    CAGwithAN=find(PercAN>=1);
    if numel(CAGwithAN)==1 && CAGwithAN==1
        ActiveNeuronsRatio=0.5; % INPUT
    else
        ActiveNeuronsRatio=1; % INPUT
    end
    ...
    ErrorClass=zeros(numel(CAGwithAN),NensemblesTotal);
    ...
    [~,CAG_THindx]=min(MeanErrorNensembles);
