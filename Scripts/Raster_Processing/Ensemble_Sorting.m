...
for c=1:NC
    ...
    if ismember('Clustering',NamesFields)   % Significant Coactivity
        THR=NN_data.Peaks.Threshold;                        % Coactivity Threshold
...
CounterColors=TotalNG+1; % A color for each group and all neurons together
...
ColorState(end,:)=[ 0.70588,0,0.78431]; % DEEP PURPLE
% SHOW COLORMAP:
colormapfig=figure('Position',[800 526 230 75],...
    'Name','Ensemble Colormap');
image(1:CounterColors); colormap(ColorState)
...
%   Sorted
Plot_Raster_Ensembles(OriginalExperiment,Indexes(New_Order_Clustering),5,fs);   % Sorted Raster
% Plot_State_Colors;
...
for c=1:NCplot
    NameSave{c,1}=Experiment(2:end);            % Experiment ID 
...
if CummFrames==TotalFrames && NC==1
    ...
    THR*ones(NCplot,1),RateSharedNeurons,RateActivityLength,...
    'VariableNames',HeadersFeatures);
else
    Tensembles=table(NameSave,Condition_Names',cell2mat(NGroups'),Ntransition,...
    THR*ones(NC,1),RateSharedNeurons,RateActivityLength,...
    'VariableNames',HeadersFeatures);
end
disp(Tensembles);
disp(Tdetailed);
