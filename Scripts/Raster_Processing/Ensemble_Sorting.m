...
for c=1:NC
    ...
    if ismember('Clustering',NamesFields)   % Significant Coactivity
        THR{c}=NN_data.Peaks.Threshold;                        % Coactivity Threshold
...
if CummFrames==TotalFrames && NC==1
    ...
    THR{c}*ones(NCplot,1),RateSharedNeurons,RateActivityLength,...
    'VariableNames',HeadersFeatures);
else
    Tensembles=table(NameSave,Condition_Names',cell2mat(NGroups'),Ntransition,...
    THR{c}*ones(NC,1),RateSharedNeurons,RateActivityLength,...
    'VariableNames',HeadersFeatures);
end
disp(Tensembles);
disp(Tdetailed);
