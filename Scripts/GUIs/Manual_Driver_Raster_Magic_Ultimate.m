...
% CONTROLS VIDEO/ CONDITION
% Left Video Manager
leftvid = uicontrol('Style','pushbutton',...
'String','<- [V]',...
'Callback',@CTRL_VIDS);
leftvid.Position=[0.01,0.01,0.15,0.0444];
% Right Video Manager
rightvid = uicontrol('Style','pushbutton',...
'String','[V] ->',...
'Callback',@CTRL_VIDS);
rigthvid.Position=[];
% +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    for j=1:NVmax               % Video Loop
        ...
        if ~isempty(SIGNALS{j,i})    % Check If There's Video 
            ...
            if ~isempty(isSIGNAL)    % Check If There's Signals Detected
                ...
                neuron=isSIGNAL(indx_neuron);         % Neuron ID
                ...
                d=daux;                               % Driver++
                % Plot Data +++++++++++++++++++++++++++++++++++++++++++++++
                Plot_Data_Now();        % Where Magic Happens: plot figure
                % Pause **************************************************
                if indx_neuron<=length(isSIGNAL)
                    % Maybe OK button or Navigator
                    pause;
                ...
                activeINDX=find(sum(R(isSIGNAL,:),2)>0);     % Indexes of Detected Ones
                yesisSIGNAL=isSIGNAL(activeINDX);            %Indexes of Active Neurons
                ...
                RASTEROK{j,i}(yesisSIGNAL,:)=R(yesisSIGNAL,:);            % Raster
                ...
                fprintf('No Signals Detected @ %s Video %i\n',Names_Conditions{i}',j)
            end
...
for i=1:NC                      % Condition Loop
    for j=1:NVmax               % Video Loop
        if ~isempty(SIGNALS{j,i})    % Check If There's Video 
            ...
            R=RASTEROK{j,i};                 % Raster        {ALL CELLS}
            X_SPARSE=zeros(size(X));         % Initialize Sparse Clean Signals
            ...
            if ~isempty(isSIGNAL)    % Check If There were Non-Detected Signals 
                ...
                recoveredINDX=find(sum(R(isSIGNAL,:),2)~=0);
                ...
                RASTEROK{j,i}(recoSIGNAL,:)=R(recoSIGNAL,:);
