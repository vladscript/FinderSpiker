...
while MoreFiles
    ...
    for c=1:NC
        ...
        if aremerged
            PosActive=find(sum(R_merged{c},2));
            RoA_POS{c}=[RoA_POS{c};sum(R_merged{c}(PosActive,:),2)./F_POS];
            NegActive=find(sum(R_nomerged{c},2));
            RoA_NEG{c}=[RoA_NEG{c};sum(R_nomerged{c}(NegActive,:),2)./F_NEG];        
        end
...
if strcmp('Yes',okbutton)
    ...
    if ~aremerged
        ...
        save([PathSave,SaveFile],'EXPS','Names_Conditions','RoA_ALL',...
            'ISIs_ALL','TranLengths_ALL',...
            'RoA_POS','ISIs_POS','TranLengths_POS',...
            'RoA_NEG','ISIs_NEG','TranLengths_NEG');
    end
